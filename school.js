document.addEventListener("DOMContentLoaded", () => {
    ...
    alreadyPlayedBtn.addEventListener("click", () => {
      ...
      window.location.href = "school.html?showArchive=true"
    })
    ...
    async function loadQuizData() {
      try {
        const response = await fetch("schoolquestions.json")
          ...
          if (month > currentMonth || (month === currentMonth && day > currentDay)) {
            ...
            item.addEventListener("click", () => {
              ...
              if (date !== currentDateKey && date !== "default") {
                window.location.href = `school.html?date=${date}`
              } else {
    ...
    function saveGameResult(finalScore) {
      ...
      // Get existing scores
      const scoresData = localStorage.getItem("schoolScores")
      ...
      localStorage.setItem("schoolScores", JSON.stringify(scores))
  
      // Mark this quiz as completed
      const completedQuizzes = getCompletedQuizzes()
      if (!completedQuizzes.includes(currentDateKey)) {
        completedQuizzes.push(currentDateKey)
        localStorage.setItem("completedSchoolQuizzes", JSON.stringify(completedQuizzes))
      }
    }
  
    // Function to get completed quizzes
    function getCompletedQuizzes() {
      const completedData = localStorage.getItem("completedSchoolQuizzes")
