document.addEventListener("DOMContentLoaded", () => {
    ...
    alreadyPlayedBtn.addEventListener("click", () => {
      ...
      window.location.href = "sport.html?showArchive=true"
    })
    ...
    async function loadQuizData() {
      try {
        const response = await fetch("sportquestions.json")
          ...
          if (month > currentMonth || (month === currentMonth && day > currentDay)) {
            ...
            item.addEventListener("click", () => {
              ...
              if (date !== currentDateKey && date !== "default") {
                window.location.href = `sport.html?date=${date}`
              } else {
    ...
    function saveGameResult(finalScore) {
      ...
      // Get existing scores
      const scoresData = localStorage.getItem("sportScores")
      ...
      localStorage.setItem("sportScores", JSON.stringify(scores))
  
      // Mark this quiz as completed
      const completedQuizzes = getCompletedQuizzes()
      if (!completedQuizzes.includes(currentDateKey)) {
        completedQuizzes.push(currentDateKey)
        localStorage.setItem("completedSportQuizzes", JSON.stringify(completedQuizzes))
      }
    }
  
    // Function to get completed quizzes
    function getCompletedQuizzes() {
      const completedData = localStorage.getItem("completedSportQuizzes")
