document.addEventListener("DOMContentLoaded", () => {
    ...
    alreadyPlayedBtn.addEventListener("click", () => {
      ...
      window.location.href = "tune.html?showArchive=true"
    })
    ...
    function showAlreadyPlayedPopup() {
      ...
      alreadyPlayedTitle.textContent = `You've Already Played the ${displayDate} Quiz!`
    ...
    async function loadQuizData() {
      try {
        const response = await fetch("tunequestions.json")
          ...
          if (month > currentMonth || (month === currentMonth && day > currentDay)) {
            ...
            item.addEventListener("click", () => {
              ...
              if (date !== currentDateKey && date !== "default") {
                window.location.href = `tune.html?date=${date}`
              } else {
    ...
    function saveGameResult(finalScore) {
      ...
      // Get existing scores
      const scoresData = localStorage.getItem("tuneScores")
      ...
      localStorage.setItem("tuneScores", JSON.stringify(scores))
  
      // Mark this quiz as completed
      const completedQuizzes = getCompletedQuizzes()
      if (!completedQuizzes.includes(currentDateKey)) {
        completedQuizzes.push(currentDateKey)
        localStorage.setItem("completedTuneQuizzes", JSON.stringify(completedQuizzes))
      }
    }
  
    // Function to get completed quizzes
    function getCompletedQuizzes() {
      const completedData = localStorage.getItem("completedTuneQuizzes")
